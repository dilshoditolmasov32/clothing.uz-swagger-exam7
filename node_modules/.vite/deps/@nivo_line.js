...
var Delaunator = class _Delaunator {
  ...
  update() {
    ...
    for (let k3 = 0, xp, yp; k3 < this._ids.length; k3++) {
      ...
      const x2 = coords[2 * i3];
      const y = coords[2 * i3 + 1];
      if (k3 > 0 && Math.abs(x2 - xp) <= EPSILON && Math.abs(y - yp) <= EPSILON) continue;
      xp = x2;
      yp = y;
...
var Voronoi = class {
  ...
  render(context) {
    ...
    for (let i3 = 0; i3 < hull.length; ++i3) {
      ...
      if (p2) this._renderSegment(x2, y, p2[0], p2[1], context);
    }
  ...
  *cellPolygons() {
  ...
  contains(i3, x2, y) {
    if ((x2 = +x2, x2 !== x2) || (y = +y, y !== y)) return false;
  ...
  *neighbors(i3) {
  ...
  _clipSegment(x0, y0, x1, y1, c0, c1) {
    while (true) {
      ...
      let x2, y, c = c0 || c1;
      if (c & 8) x2 = x0 + (x1 - x0) * (this.ymax - y0) / (y1 - y0), y = this.ymax;
      else if (c & 4) x2 = x0 + (x1 - x0) * (this.ymin - y0) / (y1 - y0), y = this.ymin;
      else if (c & 2) y = y0 + (y1 - y0) * (this.xmax - x0) / (x1 - x0), x2 = this.xmax;
      else y = y0 + (y1 - y0) * (this.xmin - x0) / (x1 - x0), x2 = this.xmin;
      if (c0) x0 = x2, y0 = y, c0 = this._regioncode(x0, y0);
      else x1 = x2, y1 = y, c1 = this._regioncode(x1, y1);
    }
  }
  _clipInfinite(i3, points, vx0, vy0, vxn, vyn) {
    let P3 = Array.from(points), p2;
    if (p2 = this._project(P3[0], P3[1], vx0, vy0)) P3.unshift(p2[0], p2[1]);
    if (p2 = this._project(P3[P3.length - 2], P3[P3.length - 1], vxn, vyn)) P3.push(p2[0], p2[1]);
    if (P3 = this._clipFinite(i3, P3)) {
      for (let j3 = 0, n3 = P3.length, c0, c1 = this._edgecode(P3[n3 - 2], P3[n3 - 1]); j3 < n3; j3 += 2) {
        ...
        if (c0 && c1) j3 = this._edge(i3, c0, c1, P3, j3), n3 = P3.length;
      }
    } else if (this.contains(i3, (this.xmin + this.xmax) / 2, (this.ymin + this.ymax) / 2)) {
  ...
  _project(x0, y0, vx, vy) {
    ...
    if (vy < 0) {
      ...
      if ((c = (this.ymin - y0) / vy) < t3) y = this.ymin, x2 = x0 + (t3 = c) * vx;
    } else if (vy > 0) {
      if (y0 >= this.ymax) return null;
      if ((c = (this.ymax - y0) / vy) < t3) y = this.ymax, x2 = x0 + (t3 = c) * vx;
    }
    if (vx > 0) {
      if (x0 >= this.xmax) return null;
      if ((c = (this.xmax - x0) / vx) < t3) x2 = this.xmax, y = y0 + (t3 = c) * vy;
    } else if (vx < 0) {
      if (x0 <= this.xmin) return null;
      if ((c = (this.xmin - x0) / vx) < t3) x2 = this.xmin, y = y0 + (t3 = c) * vy;
    }
    return [x2, y];
  }
  _edgecode(x2, y) {
...
var Delaunay = class _Delaunay {
  ...
  *neighbors(i3) {
  ...
  find(x2, y, i3 = 0) {
    if ((x2 = +x2, x2 !== x2) || (y = +y, y !== y)) return -1;
    const i0 = i3;
    let c;
    while ((c = this._step(i3, x2, y)) >= 0 && c !== i3 && c !== i0) i3 = c;
    return c;
  }
  _step(i3, x2, y) {
    ...
    do {
      ...
      if (e3 === -1) {
        ...
        if (e3 !== t3) {
          if (pow(x2 - points[e3 * 2], 2) + pow(y - points[e3 * 2 + 1], 2) < dc) return e3;
        }
        break;
      }
    } while (e3 !== e0);
...
var O = function(e3) {
  var i3 = e3.nodes, l2 = e3.width, u = e3.height, a2 = e3.x, d = e3.y, c = e3.onMouseEnter, h = e3.onMouseMove, m = e3.onMouseLeave, y = e3.onClick, g3 = e3.debug, b2 = (0, import_react.useRef)(null), k3 = (0, import_react.useState)(null), C2 = k3[0], L = k3[1], w3 = x({ points: i3, x: a2, y: d, width: l2, height: u, debug: g3 }), W = w3.delaunay, D2 = w3.voronoi, M = (0, import_react.useMemo)(function() {
    if (g3 && D2) return D2.render();
  }, [g3, D2]), O2 = (0, import_react.useCallback)(function(n3) {
    if (!b2.current) return [null, null];
...
var Ee = function(e3) {
  ...
  var Oe2 = (0, import_react2.useCallback)(function(e4) {
    ...
    if (!Ai(ne2.left, ne2.top, oe2, te2, r3, n3)) return null;
